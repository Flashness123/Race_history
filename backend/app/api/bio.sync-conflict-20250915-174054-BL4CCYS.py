...
outer = APIRouter(prefix="/bio", tags=["bio"])
...
def get_my_bio(db: Session = Depends(get_db), claims: dict = Depends(get_current_user_claims)):
    ...
    img = user.profile_image_url or DEFAULT_AVATAR
    return BioOut(
...
def update_my_bio(payload: BioIn, db: Session = Depends(get_db), claims: dict = Depends(get_current_user_claims)):
    ...
    # sanitize nationality (avoid DB length errors)
...
def list_riders(db: Session = Depends(get_db)):
    users = db.execute(select(User.id, User.display_name, User.name, User.profile_image_url)).all()
    out: list[RiderListOut] = []
    for uid, dname, uname, img in users:
        ...
        out.append(RiderListOut(
            id=uid,
            name=name,
            nationality=db.scalar(select(Bio.nationality).where(Bio.user_id == uid)),
            achievements_count=count,
            profile_image_url=img or DEFAULT_AVATAR,
        ))
    return out
