...
def include_object(object, name, type_, reflected, compare_to):
    # Only consider our declarative tables; ignore everything else (PostGIS, TIGER, topology, etc.)
    if type_ == "table":
        return name in APP_TABLES
    if type_ in ("index", "unique_constraint", "foreign_key_constraint"):
        # Only include constraints for our tables
        parent_table = getattr(object, "table", None)
        if parent_table is not None:
            return parent_table.name in APP_TABLES
    return True
